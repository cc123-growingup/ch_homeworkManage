...
<body>
	...
	<div class="container">
		...
		<form:form action="homework/addHomework?act=update" method="post" class="form-horizontal" modelAttribute="homework"  enctype="multipart/form-data" >
			...
			<div class="form-group has-success">
				  ...
				  <div class="col-sm-4 col-md-4">
		  				<input type="file" placeholder="请选择要上传的文件"  name="homework_file" class="form-control" />
							<!-- 显示已上传文件名及下载链接 -->
							<c:if test="${not empty homework.homework_file}">
    							<div>
        							<a href="homework/download?id=${homework.id}&filename=${homework.homework}" target="_blank">
            							${homework.homework}
        							</a>
        							<!-- 若有图片预览功能则保留 -->
        							<%-- <img src="static/multifile/${homework.homework_file}" style="height: 50px; width: 50px; display: block;"> --%>
    							</div>
							</c:if>
				  				</div>
			 </div>
